...
// 发送者兜底展示文案
const (
	DeletedUserText = "用户不存在或已注销" // 发送者已注销（软删后 Preload 未命中）时的展示文案
	SystemNickname  = "系统通知"      // 系统通知（type=4）的发送者展示文案
)
...
type Message struct {
	...
	FromUserID uint      `gorm:"not null" json:"from_user_id"`          // 发送者用户 ID（触发行为的人）；0=系统通知（无真实发送者）
