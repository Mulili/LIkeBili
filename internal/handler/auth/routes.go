...
//   - rdb/db/jwt/tokenTTL/minio/favorite/sendCoins：Service 的全部依赖，由 main 构造后注入
//
// 依赖注入链：main 构造依赖 → RegisterRoutes 组装 repo/svc/handler → 注册路由。
func RegisterRoutes(rg *gin.RouterGroup, rdb *redis.Client, db *gorm.DB, jwt *jwtlib.JWT, tokenTTL time.Duration, minio *storage.MinIO, favorite *favoritesRepo.Repository, sendCoins *svccoin.Service) {
	repo := authrepo.NewRepository(db)                                                       // 1. 用户数据仓库（依赖 db）
	svc := authserver.NewService(repo, rdb, jwt, tokenTTL, minio, favorite, sendCoins) // 2. 业务服务（7 个依赖）
